...
function retornarVendas(clienteID, writebleStream, callback){
    connection.query(`SELECT cliente.clienteID, valor, dataRealizacao, saldo FROM cliente JOIN venda ON cliente.clienteID = venda.clienteID WHERE cliente.clienteID = ${clienteID}`, (err, results) => {
        writebleStream.write(JSON.stringify(results));
        callback();
    });
}
